...
# catch if the user's clicks on the window's upper-right 'X' to ensure they want to close
def ask_quit(self):
...
#    dirPrefix = "\\\\?\\"
    ...
    # copy all files that have been changed in last 24 hours from srcdir to dstdir
...
##    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce the user to fill in all fields
##        conn = sqlite3.connect('db_filebackup.db')
##        with conn:
##            cursor = conn.cursor()
##            # Check the database for the existence of the fullname, if so we will alert user and disregard request
##            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_filebackup WHERE col_fullname = '{}'""".format(var_fullname))
##            count = cursor.fetchone()[0]
##            chkName = count
##            if chkName == 0: # if this is 0 then there is no existence of the fullname and we can add new data
##                print("chkName: {}".format(chkName))
##                cursor.execute("""INSERT INTO tbl_filebackup (col_fname,col_lname,col_fullname,col_phone,col_email) VALUES (?,?,?,?,?)""",
...
##            else:
##                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".format(var_fullnameK))
##                onclear(self) # call the function to clear all of the textboxes
##        conn.commit()
##        conn.close()
##    else:
...
##        if count < 1:
##            cur.execute("""INSERT INTO tbl_filebackup (col_fname,col_lname,col_fullname,col_phone,col_email) VALUES (?,?,?,?,?)""", (data))
##            conn.commit()
##    conn.close()
##                                                                                                                                    
...
##    with conn:
##        cursor = conn.cursor()
##        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email FROM tbl_filebackup WHERE col_fullname = (?)""",[value])
...
##                with conn:
##                    cursor = conn.cursor()
##                    cursor.execute("""DELETE FROM tbl_filebackup WHERE col_fullname = '{}'""".format(var_select))
...
##    with conn:
##        cursor = conn.cursor()
##        cursor.execute("""SELECT COUNT(*) FROM tbl_filebackup""")
##        count = cursor.fetchone()[0]
##        i = 0
##        while i < count:
##            cursor.execute("""SELECT col_fullname FROM tbl_filebackup""")
...
##    var_email = self.txt_email.get().strip()
##    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that therer is data present
##        conn = sqlite3.connect('db_filebackup.db')
##        with conn:
##            cur = conn.cursor()
##            # count records to see if the user's changes are already in
##            # the database...meaning, there are not changes to update.
##            cur.execute("""SELECT COUNT(col_phone) FROM tbl_filebackup WHERE col_phone = '{}'""".format(var_phone))
##            count = cur.fetchone()[0]
##            print(count)
##            cur.execute("""SELECT COUNT(col_email) FROM tbl_filebackup WHERE col_email = '{}'""".format(var_email))
...
##                    with conn:
##                        cursor = conn.cursor()
##                        cursor.execute("""UPDATE tbl_filebackup SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_value))
##                        onClear(self)
##                        conn.commit()
##                else:
